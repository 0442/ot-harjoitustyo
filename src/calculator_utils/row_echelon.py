...
from calculator_utils.latex_generator import matrix_to_latex
from calculator_utils.rational_number import Rn
from calculator_utils.types import Matrix
from calculator_utils.matrix_utils import (
    validate_matrix,
    add_row,
...
def row_echelon(matrix: Matrix, steps: list[Step] = None) -> Matrix:
    """Takes the matrix to some row echelon form.
    Fills in the provided list `steps` with the steps taken to
    solve the row echelon form.
    ...
    steps.append(Step("Arrange matrix", matrix_to_latex(result_matrix.copy())))
    ...
    for pivot in range(mat_height):
        ...
        for row_below_i in range(pivot+1, mat_height):
            ...
            if row_below[pivot_col_i] != 0:
                ...
                steps.append(Step(desc, matrix_to_latex(result_matrix.copy())))
            else:
...
def reduced_row_echelon(matrix: Matrix, steps: list[Step] = None) -> Matrix:
    """Takes the matrix to reduced row echelon form.
    Fills in the provided list `steps` with the steps taken to
    solve the reduced row echelon form.
    ...
    for pivot in range(mat_height):
        ...
        for row_below_i in range(pivot+1, mat_height):
            ...
            if row_below[pivot_col_i] != 0:
                ...
                un_reversed = list(reversed(rev_matrix.copy()))
                steps.append(Step(desc, matrix_to_latex(un_reversed)))
