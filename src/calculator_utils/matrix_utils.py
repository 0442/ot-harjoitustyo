...
from calculator_utils.types import Num, Matrix, RowVector
...
def add_row(row1: RowVector, row2: RowVector, scalar: Num = 1) -> RowVector:
    ...
    result_vector = [elem1 + elem2 *
                     scalar for elem1, elem2 in zip(row1, row2)]
    return result_vector


def subtract_row(row1: RowVector, row2: RowVector, scalar: Num = 1) -> RowVector:
    """Returns row1 - (scalar * row2).
...
def multiply_row(row: RowVector, scalar: Num) -> RowVector:
...
def all_leading_nums_to_one(matrix: Matrix, steps: list[Step]) -> Matrix:
    ...
    for row_i, row in enumerate(matrix):
        ...
        result[row_i] = n_row
        if n_row != row:
            desc = ("Turn the leading number to $1$ "
                    f"by multiplying $R_{row_i+1}$")
            steps.append(Step(desc, matrix_to_latex(result.copy())))
    return result
